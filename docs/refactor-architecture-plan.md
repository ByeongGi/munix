...
- 다음 store 후보는 `workspace-tree-slice.ts`의 orchestration 정리 또는
  `tab-slice.ts`의 side effect 경계 정리다.
...
2. **Tab Store side effect 경계 정리**
   - 대상: `store/slices/tab-slice.ts`
   - 이유: 배열 계산은 `tab-helpers.ts`로 빠졌지만 editor/recent side effect와
     workspace tree mirror 분기가 아직 slice action에 남아 있다.
