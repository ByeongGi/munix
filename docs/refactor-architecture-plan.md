...
2. **Tab Store helper 테스트 추가**
   - 대상: `store/slices/tab-helpers.ts`
   - 이유: 단일 pane tab 배열 계산이 pure helper로 분리됐으므로 단위 테스트를
     붙이기 좋다.
   - 검증: `pnpm --dir munix exec vitest run src/store/slices/tab-helpers.test.ts`,
     `pnpm --dir munix build`
