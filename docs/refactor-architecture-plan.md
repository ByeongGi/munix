...
- pane tab mutation commit과 active pane mirror-only commit 흐름을 이름 붙인
  helper로 정리했다.
- `workspace-tree-slice.ts`는 약 763줄로 줄었지만 아직 일부 split/close
  orchestration이 남아 있다.
...
- `tab-helpers.test.ts`를 추가해 단일 pane tab 배열 계산 회귀 테스트를
  보강했다.
- 다음 store 후보는 `workspace-tree-slice.ts`의 split/close orchestration 정리
  또는 tab side effect/mirror 경계 추가 테스트다.
...
1. **Workspace Store split/close orchestration 추가 정리**
   - 대상: `store/slices/workspace-tree-slice.ts`
   - 이유: pane tab commit 흐름은 정리됐지만 split/close action 조립 흐름이
     아직 길다.
   - 선행: 현재 Vitest를 유지하면서 commit helper 이름과 책임을 정리한다.
   - 검증: `pnpm --dir munix exec vitest run src/store/slices/workspace-tree-slice.test.ts`,
     `pnpm --dir munix build`

2. **Tab Store action 테스트 검토**
