...
- `workspace-tree-slice.ts`는 약 785줄로 줄었지만 아직 action orchestration이
  많다.
- 다음 store 후보는 `workspace-tree-slice.ts`의 action별 mutation helper 추가
  분리 또는 `tab-slice.ts`(약 408줄) 정리다.
...
2. **Tab Store slice 분리 준비**
   - 대상: `store/slices/tab-slice.ts`
   - 이유: workspace helper 분리 이후 다음으로 큰 store slice다.
   - 검증: 관련 Vitest 확인 후 `pnpm --dir munix build`
