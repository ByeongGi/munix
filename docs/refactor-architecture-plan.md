...
3. [x] markdown read/write side effect를 hook 경계로 분리한다.
...
- `inactive-pane-editor.tsx` 내부의 read/write/autosave 루프를
  `use-inactive-pane-document-loader`와 `use-inactive-pane-autosave`로 분리했다.
- `inactive-pane-editor.tsx`는 255줄에서 약 122줄로 줄었고, Tiptap 구성과
  inactive editor UI 조립 중심이 됐다.
- 저장 충돌, pending save, blur flush, rename 전 idle wait 흐름은
  `use-inactive-pane-autosave`가 현재 소유한다.
...
- [x] palette command builder를 file/tab/sidebar/theme/app/workspace/vault
  명령 그룹별 파일로 분리.
- [x] settings panel을 general/editor/shortcut/advanced/about/vault 파일로
  추가 분리.

남은 후보:

- `palette-command-builders.ts`는 public composition entry로만 남았다.
- `settings-panels.tsx`는 panel re-export entry로만 남았다.

## Phase 4.6: File Tree 폴더링

`file-list.tsx`와 `file-tree-inner.tsx`가 selection, keyboard, root drop,
row drop, drag preview, empty UI를 동시에 담당하던 상태를 분리했다.

완료:

- [x] file tree empty placeholder를 작은 컴포넌트로 분리.
- [x] folder expand/reveal 상태를 `use-file-list-expansion`으로 분리.
- [x] focused/selected/range selection 상태를 `use-file-list-selection`으로 분리.
- [x] keyboard navigation/delete/rename dispatch를 `use-file-list-keyboard`로 분리.
- [x] root empty-area drop 처리를 `use-file-list-root-dnd`로 분리.
- [x] row drag/drop 규칙을 `use-flat-tree-row-dnd`로 분리.
- [x] DOM drag preview 생성을 `drag-preview.ts`로 분리.

완료 메모:

- `file-list.tsx`는 363줄에서 약 202줄로 줄었고, list shell과 Virtuoso 조립
  중심이 됐다.
- `file-tree-inner.tsx`는 340줄에서 약 187줄로 줄었고, row 표시와 click
  dispatch 중심이 됐다.
- `pnpm --dir munix lint`, `pnpm --dir munix build` 통과. 기존 Vite
  dynamic/static import 및 chunk size warning은 계속 남아 있다.
...
3. [x] workspace tree의 pure tree/tab helper를 slice 밖으로 분리한다.
4. [x] pane tab close/pin 계산을 pure helper로 추가 분리한다.
5. [x] path remove/rename 계산을 pure helper로 추가 분리한다.
6. [x] split/move tab 계산을 pure helper로 추가 분리한다.
7. slice action 이름을 domain verb 중심으로 점검한다.
8. 기존 테스트가 있는 slice는 함께 실행한다.

현재 상태:

- `store/slices/workspace-tree-helpers.ts`를 추가해 split build, node replace,
  pane remove, empty pane prune, global capture, tab removal neighbor 계산,
  markdown basename 계산을 분리했다.
- pane tab close/close others/close after/close all/toggle pinned 계산도
  helper로 분리했다.
- path 기반 remove/rename 계산도 helper로 분리했다.
- split/move tab 계산도 helper로 분리했다.
- pane tab mutation commit과 active pane mirror-only commit 흐름을 이름 붙인
  helper로 정리했다.
- `workspace-tree-slice.ts`의 split/close/split-move tree 계산을
  `workspace-tree-helpers.ts`로 추가 분리했다. slice action은 상태 capture,
  commit, editor side effect 중심으로 읽히도록 정리했다.
- `tab-helpers.ts`를 추가해 단일 pane tab 배열 계산(close, close others,
  close after, close all, promote empty, rename/remove path, reorder)을
  분리했다.
- `tab-slice.ts`의 editor open/close side effect 호출도
  `syncEditorToActiveTab`으로 묶어 active tab 반영 의도를 명확히 했다.
- `tab-slice.ts`의 active pane mirror patch 분기도 `patchActivePaneTabs`로
  정리했다.
- `tab-helpers.test.ts`를 추가해 단일 pane tab 배열 계산 회귀 테스트를
  보강했다.
- 다음 store 후보는 tab side effect/mirror 경계 action 테스트 또는
  `workspace-tree-slice.ts`의 index 기반 reorder 중복 정리다.
...
1. **Tab Store action 테스트 검토**
   - 대상: `store/slices/tab-slice.ts`
   - 이유: helper 테스트는 추가됐지만 editor/recent side effect와 workspace
     mirror 분기는 action 단위 테스트가 없다.
   - 검증: 관련 Vitest 추가 후 `pnpm --dir munix build`

2. **Workspace Store reorder/index helper 정리**
   - 대상: `store/slices/workspace-tree-slice.ts`
   - 이유: split/close/split-move orchestration은 분리됐고, 이제 index 기반
     reorder 계산이 slice 안에 일부 남아 있다.
   - 선행: 현재 Vitest를 유지하면서 순수 계산 helper와 action commit 경계를
     분리한다.
   - 검증: `pnpm --dir munix exec vitest run src/store/slices/workspace-tree-slice.test.ts`,
     `pnpm --dir munix build`
