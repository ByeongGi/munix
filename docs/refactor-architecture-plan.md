...
- `workspace-tree-slice.ts`는 약 785줄로 줄었지만 아직 action orchestration이
  많다.
...
1. **Workspace Store orchestration 추가 정리**
   - 대상: `store/slices/workspace-tree-slice.ts`
   - 이유: pure helper는 대부분 분리됐지만 collapse/mirror commit 흐름이
     slice 내부에 남아 있다.
