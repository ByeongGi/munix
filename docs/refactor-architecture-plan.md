...
- 다음 store 후보는 `workspace-tree-slice.ts`의 orchestration 정리 또는
  `tab-slice.ts`의 workspace tree mirror 분기 정리다.
...
2. **Tab Store workspace mirror 분기 정리**
   - 대상: `store/slices/tab-slice.ts`
   - 이유: 배열 계산과 editor side effect는 정리됐지만 workspace tree mirror
     분기가 일부 action에 남아 있다.
