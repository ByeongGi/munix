...
6. slice action 이름을 domain verb 중심으로 점검한다.
7. 기존 테스트가 있는 slice는 함께 실행한다.
...
- `workspace-tree-slice.ts`는 약 824줄로 줄었지만 아직 move/split action body가
  크다.
...
1. **Workspace Store split/move helper 추가 분리**
   - 대상: `store/slices/workspace-tree-slice.ts`
   - 이유: pure helper는 분리됐지만 split/move action body가 아직 길다.
   - 선행: 현재 Vitest를 유지하면서 action별 작은 helper extraction부터 진행.
