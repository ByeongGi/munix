...
1. **Workspace Store action helper 추가 분리**
   - 대상: `store/slices/workspace-tree-slice.ts`
   - 이유: pure helper는 분리됐지만 move/close/remove/rename action body가
     아직 길다.
...
2. **Settings panel 추가 분리**
   - 대상: `components/settings/settings-panels.tsx`
   - 이유: 패널별 UI가 한 파일에 남아 있어 설정 확장 시 충돌 가능성이 있다.
   - 검증: `pnpm --dir munix lint`, 필요 시 `pnpm --dir munix build`
