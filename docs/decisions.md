...
| [ADR-033](#adr-033-터미널-native-libghostty-runtime) | 터미널 native libghostty runtime | 2026-05-01 | 🔴 Superseded by ADR-034 |
| [ADR-034](#adr-034-터미널-xtermjs--portable-pty-runtime) | 터미널 xterm.js + portable-pty runtime | 2026-05-01 | ✅ Accepted |
...
초기 결정은 `xterm.js + portable-pty` 또는 `ghostty-web + portable-pty`를 전제로 했다.
실험 결과 `ghostty-web`은 Tauri WebView/canvas lifecycle에서 이전 프레임 잔상 문제가 반복 재현되었고,
app-level clear/reset/visibility/offscreen mount workaround로 안정화하지 못했다.

따라서 `ghostty-web` canvas renderer 접근은 폐기한다.
최종 제품 경로는 [ADR-034](#adr-034-터미널-xtermjs--portable-pty-runtime) 를 따른다.
...
## ADR-033: 터미널 native libghostty runtime

**상태:** 🔴 Superseded by ADR-034
**결정일:** 2026-05-01
**Supersedes:** [ADR-023](#adr-023-터미널-플러그인-1호)

### 컨텍스트

Munix의 통합 터미널은 처음에 Tauri WebView 안에서 `ghostty-web`을 렌더링하고 Rust `portable-pty`로 PTY를 관리하는 방식으로 구현했다. 그러나 terminal tab close/reopen, shell `exit` 후 재오픈, split pane 이동/활성 전환에서 이전 canvas/output이 잔상처럼 보이는 문제가 계속 발생했다.

시도한 완화책:

- terminal tab을 workspace persistence에서 제거
- session id와 output buffer를 분리하고 stale event 차단
- shell exit 시 buffer/session 정리
- `ghostty-web` PR #154 bootstrap blank patch 커밋 적용
- `@xterm/headless + @xterm/addon-serialize` 기반 screenState 복원 도입
- terminal viewport `visibility:hidden` 및 offscreen staging mount 적용

위 조치 후에도 증상이 남았으므로 문제를 app-level buffer 재생 문제가 아니라 WebView/canvas 렌더러 lifecycle 및 `ghostty-web` 통합 경계 문제로 본다.

`weedonandscott/trolley`는 `ghostty-web`을 쓰지 않고 native `libghostty`를 직접 붙인다.

- macOS: Swift/AppKit `NSWindow` + `NSView` + Metal surface
- Linux: Zig + GLFW + OpenGL
- Windows: Zig + Win32 + OpenGL

즉 WebView/canvas를 우회하고, `libghostty`가 VT parsing, PTY management, GPU rendering, font shaping, input encoding을 모두 담당한다.

### 결정

Munix의 장기 터미널 구현은 `trolley` 방식을 따른다.

1. **WebView 내부 terminal renderer는 폐기한다.**
   `ghostty-web`, xterm.js DOM/canvas renderer, app-level screenState workaround는 장기안이 아니다.
2. **터미널은 native `libghostty` surface로 구현한다.**
   Tauri/Rust 쪽에서 platform-specific native view를 붙이고, React는 tab/pane metadata와 focus/open/close orchestration만 담당한다.
3. **`trolley`는 코드 참조 구현으로 사용한다.**
   직접 의존성으로 가져오기보다 runtime bootstrap, config generation, surface lifecycle, input event mapping, font/theme bundling 방식을 참고한다.
4. **PoC는 macOS부터 진행한다.**
   Munix 개발/사용 1차 환경이 macOS이고, trolley macOS runtime은 Swift/AppKit + Metal surface라 문제 재현 지점인 WebView canvas를 완전히 피한다.
5. **기존 `portable-pty` IPC 터미널은 deprecated path로 둔다.**
   native terminal PoC가 안정화되기 전까지 기능 플래그 또는 dev-only fallback으로만 유지한다.
...
| 후보 | 설명 | 장점 | 단점 |
|---|---|---|---|
| Native terminal window | Tauri에서 별도 native terminal window/runtime 실행 | 구현 난이도 낮음, trolley와 가장 유사 | workspace pane 안에 embed 불가 |
| Native child view embed | Tauri macOS window에 `NSView` 기반 libghostty surface를 붙임 | 진짜 통합 terminal 가능 | Tauri plugin/native view 설계 필요 |

2026-05-01 실험에서 Tauri main webview의 `NSView`에 child `NSView` placeholder를 붙이고 React pane bounds와 동기화하는 경로를 검증했다. 따라서 별도 native window PoC는 건너뛰고, macOS pane embed를 기본 경로로 삼는다.

macOS 구현 경계는 다음처럼 둔다.

- Rust/Tauri command: vault cwd 검증, terminal instance id 관리, parent `NSView` 획득, React pane bounds 동기화
- Swift/AppKit bridge: `NSView` subclass, `NSTextInputClient`, key/mouse/scroll/focus/IME/clipboard mapping, `ghostty_app_tick`, `ghostty_surface_*` lifecycle
- `libghostty`: PTY, terminal state, renderer, font shaping, Metal surface

Rust에서 `ghostty_surface_key`/mouse/IME를 직접 모두 감싸는 방식은 입력/IME 품질과 유지보수 비용 때문에 장기 기본안으로 두지 않는다. Rust는 Swift bridge를 붙이는 host 역할에 집중한다.

### 결과

**긍정:**

- WebView canvas 잔상 문제를 구조적으로 제거
- Ghostty 본체 품질(PTY, shaping, renderer, input encoding)을 직접 사용
- starship/p10k/nerd font/TUI 호환성을 web terminal보다 높게 기대 가능

**부정:**

- React 컴포넌트 교체 수준이 아니라 native runtime 작업
- macOS/Linux/Windows별 구현 분기 필요
- Tauri window/pane embed와 focus/input routing 난이도 상승
- 배포 파이프라인에 Zig/Swift/libghostty 빌드 산출물이 추가될 수 있음
- macOS 개발 환경은 Xcode Metal Toolchain이 필요함. `xcrun metal`/`metallib`이 동작하지 않으면 `libghostty` static build가 실패한다.

### 참고

- trolley: https://github.com/weedonandscott/trolley
- trolley macOS runtime: https://github.com/weedonandscott/trolley/blob/master/runtime/macos/Sources/main.swift
- trolley Linux runtime: https://github.com/weedonandscott/trolley/blob/master/runtime/src/platform/linux.zig
- trolley build.zig: https://github.com/weedonandscott/trolley/blob/master/runtime/build.zig
- Ghostty/libghostty: https://github.com/ghostty-org/ghostty
- 관련 스펙: [specs/terminal-spec.md](./specs/terminal-spec.md)

---

## ADR-034: 터미널 xterm.js + portable-pty runtime

**상태:** ✅ Accepted
**결정일:** 2026-05-01
**Supersedes:** [ADR-023](#adr-023-터미널-플러그인-1호), [ADR-033](#adr-033-터미널-native-libghostty-runtime)

### 컨텍스트

native `libghostty` embed PoC는 `ghostty-web` 잔상 문제를 WebView 밖으로 밀어내는 데에는 타당했지만, Tauri WebView 위에 platform native child view를 얹는 구조가 새로운 문제를 만들었다.

- context menu, command palette, modal보다 native terminal view가 위에 올라오는 z-order 문제가 생겼다.
- DevTools를 열거나 Web layout이 변해도 native child view 좌표는 Web compositor와 자동으로 함께 움직이지 않았다.
- macOS Swift/AppKit bridge, `native-libghostty` Cargo feature, Ghostty source/Zig build, platform별 embed 구현이 배포 복잡도를 크게 높였다.
- Windows/Linux에서 동일한 embedded `libghostty` surface 전략을 유지하기 어렵다.

Electron/VS Code 계열의 integrated terminal은 대체로 Web renderer + backend PTY bridge 구조를 취한다. Munix도 Tauri WebView 기반 앱이므로 overlay, split pane, palette, DevTools까지 같은 compositor 안에 두는 편이 제품 구조에 맞다.

### 결정

Munix 터미널은 `@xterm/xterm` renderer와 Rust `portable-pty` backend로 구현한다.

1. **`ghostty-web`은 제거한다.**
   반복된 canvas 잔상 문제의 직접 재현 경로였으므로 제품 경로에서 제외한다.
2. **native `libghostty` embed 경로를 제거한다.**
   `terminal_native_*` IPC, Swift/AppKit bridge, `native-libghostty` feature, Ghostty build wrapper를 삭제한다.
3. **xterm.js를 Web UI 내부 renderer로 사용한다.**
   context menu, command palette, modal, DevTools가 같은 Web compositor와 CSS z-index 안에서 동작하게 한다.
4. **PTY는 기존 Rust `portable-pty` IPC를 유지한다.**
   vault cwd 검증과 shell lifecycle은 backend에서 유지하고, renderer만 xterm.js로 교체한다.
5. **terminal session은 workspace persistence에 저장하지 않는다.**
   tab close 시 PTY를 종료하고, tab switch/remount 중에는 tab id 기준 session registry와 screen state cache로 연결을 유지한다.

### 결과

**긍정:**

- overlay/z-index/DevTools 좌표 문제가 구조적으로 사라진다.
- macOS/Windows/Linux가 같은 terminal architecture를 공유한다.
- Ghostty/Zig/Swift/native view build pipeline을 제거해 개발/배포 복잡도가 줄어든다.
- VS Code 방식에 가까운 검증된 조합이다.

**부정:**

- Ghostty/libghostty 수준의 GPU renderer, shaping, terminal fidelity를 직접 쓰지는 못한다.
- xterm.js의 IME/TUI/폰트 품질 한계는 실제 사용 테스트로 지속 확인해야 한다.
- `ghostty-web`이 제공하던 Ghostty parser 계열 이점은 포기한다.

### 구현

- Frontend: `@xterm/xterm`, `@xterm/addon-fit`, `@xterm/addon-web-links`
- Session cache: `@xterm/headless`, `@xterm/addon-serialize`
- Backend: Rust `portable-pty`
- IPC: `terminal_spawn`, `terminal_write`, `terminal_resize`, `terminal_kill`
