...
> **2026-05-01 갱신**: 터미널은 ADR-033에 따라 WebView renderer(`ghostty-web`/xterm.js) 경로를 폐기하고 native `libghostty` runtime으로 전환한다.
> 플러그인 시스템(ADR-022)은 v1.1 핵심 인프라 후보로 유지하되, 터미널 PoC는 플러그인 시스템을 기다리지 않고 native runtime으로 먼저 검증한다.
...
- [ ] 🆕 **플러그인 시스템 (Extism WASM)** — [ADR-022](./decisions.md#adr-022-플러그인-시스템-extism-wasm) / [specs/plugin-spec.md](./specs/plugin-spec.md)
  ...
  - **2-3주 작업** — v1.1 핵심 인프라
- [ ] 🆕 **터미널 native `libghostty` runtime** — [ADR-033](./decisions.md#adr-033-터미널-native-libghostty-runtime) / [specs/terminal-spec.md](./specs/terminal-spec.md)
  - `trolley` 스타일 native `libghostty` surface 채택
  - T0: Tauri main `NSView` 아래 child `NSView` placeholder attach/remove/bounds sync 검증 완료
  - T1: macOS Swift/AppKit bridge skeleton 연결 완료 (`native-libghostty` feature)
  - T2: Swift `TerminalSurfaceView` 내부에 `libghostty` surface, PTY, resize, focus, key/mouse/scroll/clipboard bridge 연결
  - T3: 실제 Tauri dev runtime에서 shell input, IME, TUI, split/reopen 잔상 회귀 테스트
  - 기존 `ghostty-web` + `portable-pty` 경로는 deprecated/dev-only fallback
  - **참고 구현**: [trolley](https://github.com/weedonandscott/trolley), [cmux](https://github.com/manaflow-ai/cmux), VS Code terminal
