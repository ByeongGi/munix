...
**테스트 방향:** 수동 회귀 체크리스트는 자동화 전환 대기열로만 유지한다. 최종 목표는 [test-automation-strategy.md](./test-automation-strategy.md)에 따라 모든 수동 회귀 항목을 Vitest/Rust/Playwright/Tauri native E2E로 치환하는 것이다.
...
> **2026-05-01 갱신**: 터미널은 ADR-034에 따라 `@xterm/xterm` + Rust `portable-pty`로 구현한다.
> `ghostty-web`과 native `libghostty` embed 경로는 제거했다.

**v1.0 후반 — 출시 전 추가 작업 (v1.0 스코프 내):**

- [ ] **Obsidian-style CLI + URI scheme** — 🆕 [ADR-024](./decisions.md#adr-024-cli--uri-scheme-munix) / [specs/cli-spec.md](./specs/cli-spec.md)
  - 별도 `munix-cli` 바이너리 + Rust command model/parser
  - `munix vault=Work open path="note.md" newtab` 형태 지원
  - CLI → 실행 중인 GUI 앱 local IPC command forwarding
  - `munix://open/new/daily/search` URI 처리
  - macOS symlink / Linux `~/.local/bin` / Windows redirector 설치 UX
  - 보안: vault 외부 경로 차단, 외부 URI 첫 호출 사용자 승인
...
- [ ] 🆕 **플러그인 시스템 (Extism WASM)** — [ADR-022](./decisions.md#adr-022-플러그인-시스템-extism-wasm) / [specs/plugin-spec.md](./specs/plugin-spec.md)
  ...
  - **2-3주 작업** — v1.1 핵심 인프라
- [x] 🆕 **터미널 xterm.js runtime** — [ADR-034](./decisions.md#adr-034-터미널-xtermjs--portable-pty-runtime) / [specs/terminal-spec.md](./specs/terminal-spec.md)
  - `@xterm/xterm` renderer + Rust `portable-pty` backend
  - `@xterm/addon-fit` resize 동기화
  - `@xterm/addon-web-links` URL link handling
  - tab id 기준 session registry와 screen state cache 유지
  - native `libghostty` bridge, `ghostty-web`, wrapper build script 제거
  - **참고 구현**: VS Code terminal architecture
- [ ] 🆕 **CLI 계층 2** — `tags`, `backlinks`, `outline`, `properties`, shell completion (zsh/bash/fish), TUI mode
...
- [ ] 정규식 검색 안전성 개선
- [ ] 🆕 **native terminal embed 고도화** — macOS child view embed 안정화 후 Linux/Windows runtime 검토 (ADR-033)
- [ ] 🆕 **CLI 계층 3** — 안정화된 local JSON-RPC API, workspace/history/plugin/theme 명령, x-callback-url
...
- [ ] 관련 수동 체크리스트 항목을 자동화 테스트로 이전하거나, 자동화 불가 사유와 만료 조건 기록
...
**문서 버전:** v3.7
**작성일:** 2026-04-25
**최근 업데이트:** 2026-05-02 — 수동 회귀 테스트를 자동화 전환 대기열로 재정의. React render harness + Playwright render suite 기준을 Phase 완료 조건에 반영.
**남은 큰 작업:** 수동 검증 항목 자동화 전환 / Web Worker 검색 인덱스 빌드 / CSP 활성화 / CLI+URI scheme / 각 OS 빌드+코드 서명.
